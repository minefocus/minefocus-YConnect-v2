...
@property (nonatomic, strong) SFAuthenticationSession *authenticationSession API_AVAILABLE(ios(11.0));
 ...
 * 認可画面を表示する
 **/
- (void)requestAuthorization:(UIViewController *)presentingViewController
{
    NSURL *url = [self generateAuthorizationUri];
    [self openURL:url presentingViewController:presentingViewController];
}

/**
 * 認可画面を表示する
 **/
- (void)requestAuthorizationWithState:(NSString *)state presentingViewController:(UIViewController *)presentingViewController
{
 ...
 * 認可画面を表示する
 **/
- (void)requestAuthorizationWithPrompt:(NSString *)prompt presentingViewController:(UIViewController *)presentingViewController
{
 ...
 * 認可画面を表示する
 **/
- (void)requestAuthorizationWithState:(NSString *)state prompt:(NSString *)prompt nonce:(NSString *)nonce presentingViewController:(UIViewController *)presentingViewController
{
...
- (void)parseAuthorizationResponse:(NSURL *)uri handler:(YConnectAuthorizationEndpointResponseHandler)handler
{
    ...
    if ([self.responseType rangeOfString:YConnectConfigResponseTypeCodeIdTokenToken].location != NSNotFound) {
        [YConnectAuthorizationCallBackUriParser parseHybridCallbackUri:uri uriScheme:uriScheme state:self.state handler:^(NSString *authorizationCode, YConnectBearerToken *accessToken, NSString *idToken, NSError *error) {
            if (error) {
                handler(error);
                return;
            }
        ...
        return;
    } else if ([self.responseType rangeOfString:YConnectConfigResponseTypeCodeIdToken].location != NSNotFound) {
...
/**
 * iOSのバージョンに応じた方法でauthorizationエンドポイントにリクエストする
 * @param url リクエストURL
 * @param presentingViewController 遷移元のViewController
 **/
- (void)openURL:(NSURL *)url presentingViewController:(UIViewController *)presentingViewController
{
    // iOS11以上ならSFAuthenticationSessionを使って画面遷移する
    if (@available(iOS 11.0, *)) {
        self.authenticationSession = [[SFAuthenticationSession alloc] initWithURL:url callbackURLScheme:self.redirectUri completionHandler:^(NSURL * _Nullable callbackURL, NSError * _Nullable error) {
            id delegate = [[UIApplication sharedApplication] delegate];
            if (error == nil) {
            [delegate application:[UIApplication sharedApplication] openURL:callbackURL options:@{}];
            }
        }];
    ...
    // iOS9以上11未満ならSFSafariViewContollerを使って画面遷移する
    if (@available(iOS 9.0, *)) {
        self.safariViewController = [[SFSafariViewController alloc] initWithURL:url];
        [presentingViewController presentViewController:self.safariViewController animated:YES completion:nil];
        return;
    }

    NSLog(@"iOS8 or less cannot open URL with SFAuthenticationSession or SFSafariViewController");
}
