//
//  MFYConnect.h
//  MFYConnect
//
//  Created by madao23 on 2018/11/14.
//  Copyright © 2018 SCSK Corporation. All rights reserved.
//

#import <UIKit/UIKit.h>
#import "YConnect.h"
#import "YConnectUserInfoClient.h"
#import "YConnectLog.h"
#import "YConnectCheckIdClient.h"
#import "YConnectTokenClient.h"
#import "NSNull+isNull.h"

//! Project version number for MFYConnect.
FOUNDATION_EXPORT double MFYConnectVersionNumber;

//! Project version string for MFYConnect.
FOUNDATION_EXPORT const unsigned char MFYConnectVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <MFYConnect/PublicHeader.h>


